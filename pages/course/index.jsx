      ...
      <div className="course-card flex flex-col items-center md:grid md:grid-cols-2 lg:grid-cols-3 w-full md:w-fit gap-8 px-4">
        {courses.length > 0 ? (
          courses.map((course, course_id) => (
            <div
              key={course_id}
              className="card h-[431px] w-[343px] rounded-lg flex flex-col gap-2 border-x bg-white drop-shadow-xl"
              onClick={() => {
                router.push(`course/${course.course_id}`);
              }}
            >
              <div className="course-img w-full h-[240px] bg-gray-900"></div>
              <div className="course-content h-[122px] px-4 pt-2 pb-4 flex flex-col gap-1">
              ...
              <div className="course-footer border-t-[1px] flex flex-row gap-6 p-4">
